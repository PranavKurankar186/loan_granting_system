...
fieldofficer();
  // axios.get('http://localhost:8080/admin/fieldofficerlist').then((response) =>{
  //   console.log("responsedata", response.data)       
  //               setData1(response.data);
   ...
   const[field_officer_name,setFieldOfficerName]=useState();
   const[user_id,setUserId]=useState();


        const handleSubmit  = (e) => {
          var body={ loan_id,user_id,field_officer_name}
          console.log(loan_id)
          console.log(field_officer_name)
          console.log(user_id)
          axios.post(`http://localhost:8080/admin/assignfieldofficer`,body)
          .then(response => {  console.log(response.status); swal("Success", "Field Officer Assigned Successfully", "success");}) 
          .catch(error => {  console.log(error); swal("Error", "Field Officer Not Assigned", "error");})
      }
  ...
  return (         
    ...
    <div className="AFO">
      ...
      <h3 className="mt-3 mb-4">Assign Field OFficer</h3>
...
<div className="AFO_select_list">
    <label className="me-2 ms-2">Field Officer Name: </label>
    <select name='loan_officer_name' placeholder="select value"  onChange={(e) => setFieldOfficerName(e.target.value)}>
      <option></option>
      {data1?.map((e) =>{
