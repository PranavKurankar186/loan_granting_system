...
const navigate = useNavigate();
    ...
    useEffect(()=>{
        axios.get(`http://localhost:8080/admin/updateloanverification`).then((response) =>{
  ...
  return (
    <div>
      <AdminNav/>
         <h2 className="mb-4 ms-3 mt-3">Update Loan Verification List</h2>
      ...
      {data?.map((element) => {
        return (
          <tr>
            ...
            <td>{element[3]}</td>
            <td>{element[4]}</td>
            <td><a href="/viewLoanReportAdmin">report</a></td>
...
export default UpdateLoanVerification