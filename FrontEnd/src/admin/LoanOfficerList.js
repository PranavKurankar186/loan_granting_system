...
useEffect(() => {
  ...
  useEffect(()=>{
        ...
        const updateStatus=(e)=>{
         ...
         . then(response => {  console.log(response.status);swal("Success", "Updated Successfully", "success");loanofficer();}) 
         ...
         const getUserListByName = (e) => {
          ...
          axios.get(`http://localhost:8080/admin/searchbyname/` + searchUser).then((response) =>{
            console.log("responsedata", response.data)       
                    //  console.log("with mapping", item.address)
                    setData(response.data.filter( x => x.user_category=="loanofficer"));
